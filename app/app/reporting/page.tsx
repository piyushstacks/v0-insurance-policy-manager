...
export default function ReportingPage() {
  ...
  if (isLoading) return <PageLoader words={['reports', 'analytics', 'data']} label="loading" />;

  if (isMember) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[80vh] p-6 text-center bg-slate-50">
  ...
  return (
    <div className="flex flex-col p-4 md:p-8 min-h-full max-w-[1600px] mx-auto w-full pb-24 lg:pb-8 bg-slate-50/50">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4 mb-6">
        <div>
          <div className="flex items-center gap-3 mb-2">
            ...
            <div>
              <h1 className="text-3xl font-extrabold tracking-tight text-slate-900">Detailed Analysis</h1>
              <p className="text-slate-500 font-medium">Comprehensive portfolio and revenue breakdown</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm flex items-center gap-4">
          ...
          <div>
            <p className="text-sm font-semibold text-slate-500 uppercase tracking-wider mb-1">Total Book of Business (AUM)</p>
            <h2 className="text-3xl font-black text-slate-900 tabular-nums">{formatCurrency(data.totalAUM)}</h2>
          </div>
        </div>
        <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm flex items-center gap-4">
          ...
          <div>
            <p className="text-sm font-semibold text-slate-500 uppercase tracking-wider mb-1">Total Annual Premium</p>
            <h2 className="text-3xl font-black text-slate-900 tabular-nums">{formatCurrency(data.totalPremium)}</h2>
          </div>
        </div>
        <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm flex items-center gap-4">
          ...
          <div>
            <p className="text-sm font-semibold text-slate-500 uppercase tracking-wider mb-1">Total Active Policies</p>
            <h2 className="text-3xl font-black text-slate-900 tabular-nums">{data.activePolicies} / {data.totalPolicies}</h2>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden mb-6">
         <div className="p-5 border-b border-slate-100 bg-slate-50/50">
            <h3 className="text-lg font-bold text-slate-800">Customer Premium Analysis</h3>
         </div>
               ...
               <tbody className="divide-y divide-slate-100">
                  {data.customerChartData.map((cust: any, i: number) => (
                     <tr key={i} className="hover:bg-slate-50/50 transition-colors">
                        <td className="px-6 py-4 font-semibold text-slate-900">{cust.name}</td>
