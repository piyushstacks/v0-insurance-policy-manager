    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-blue-950 to-slate-900 flex items-center justify-center p-6">
      <div className="w-full max-w-md">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-muted-foreground text-sm">AI-Powered Insurance Management</p>
        </div>

        <div className="bg-card transition-colors/5 backdrop-blur border border-white/10 rounded-3xl p-8 shadow-2xl">
            ...
            <div className="space-y-6">
              <div className="text-center">
                ...
                <div className="mt-3 p-3 bg-blue-500/10 border border-blue-400/20 rounded-2xl">
                  ...
                  <p className="text-muted-foreground text-xs mt-0.5">Invited as a Member</p>
                </div>
                {expiresAt && <p className="text-muted-foreground text-xs mt-2">Expires {new Date(expiresAt).toLocaleString()}</p>}
              </div>
              <p className="text-muted-foreground text-sm text-center">Sign in or create an account to accept.</p>
              <div className="grid grid-cols-2 gap-3">
                ...
                <Button onClick={() => { setAuthMode('signup'); setState('auth_signup'); setFormError(''); }}
                  variant="outline" className="border-white/20 text-white hover:bg-card transition-colors/10 rounded-2xl h-11 font-semibold">
                  Sign Up
                </Button>
              </div>
            </div>
          ...
          {state === 'auth_login' && (
            <div className="space-y-5">
              <div>
                <button onClick={() => setState('valid')} className="text-muted-foreground hover:text-white text-sm mb-4 flex items-center gap-1">← Back</button>
              ...
              <form onSubmit={handleLoginSubmit} className="space-y-4">
                ...
                <div className="relative">
                  ...
                  <Input type="email" placeholder="you@example.com" value={email}
                    ...
                    className="pl-10 h-12 rounded-2xl bg-card transition-colors/5 border-white/20 text-white placeholder:text-muted-foreground" />
                </div>
          ...
          {state === 'auth_signup' && (
            <div className="space-y-4">
              <div>
                ...
                <p className="text-muted-foreground text-sm mt-1">Verify your email to join the team.</p>
              </div>
              <form onSubmit={handleSignupSubmit} className="space-y-3">
                ...
                <div className="relative">
                  ...
                  <Input type="text" placeholder="Full Name" value={fullName}
                    ...
                    className="pl-10 h-11 rounded-2xl bg-card transition-colors/5 border-white/20 text-white placeholder:text-muted-foreground" />
                </div>
                <div className="relative">
                  <Mail className="absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 text-muted-foreground" />
                  <Input type="email" placeholder="you@example.com" value={email}
                ...
                <div className="relative">
                  ...
                  <Input type={showPass ? 'text' : 'password'} placeholder="Min. 8 characters"
                    ...
                    className="pl-10 pr-10 h-11 rounded-2xl bg-card transition-colors/5 border-white/20 text-white placeholder:text-muted-foreground" />
              ...
              <form onSubmit={handleVerifyOTP} className="space-y-6">
                ...
                <div className="flex gap-2 justify-center" onPaste={handleOtpPaste}>
                  {otp.map((digit, i) => (
                    <input key={i} ref={el => { otpRefs.current[i] = el; }}
                      ...
                      className={`w-12 h-14 text-center text-2xl font-black border-2 rounded-2xl outline-none transition-all bg-card transition-colors/5
                ...
                <div className="flex items-center justify-between text-sm">
                  <button type="button"
                    ...
                    className="text-muted-foreground hover:text-white">← Back</button>
                  <button type="button"
                    onClick={() => sendOTP(state === 'auth_verify' ? 'signup' : 'login')}
                    disabled={cooldown > 0 || loading}
                    className="text-blue-400 hover:text-blue-300 disabled:text-foreground/90 flex items-center gap-1 font-medium">
          ...
          {state === 'success' && (
            <div className="text-center space-y-4 py-4">
              ...
              <p className="text-muted-foreground text-sm">Redirecting to your team dashboard...</p>
          ...
          {(state === 'invalid' || state === 'error') && (
            <div className="text-center space-y-4 py-4">
              ...
              <Button onClick={() => router.push('/auth/login')}
                variant="outline" className="border-white/20 text-white hover:bg-card transition-colors/10 rounded-2xl">
                Go to Login
              </Button>
            </div>
          )}
